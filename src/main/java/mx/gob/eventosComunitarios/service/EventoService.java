...
public class EventoService implements EventoInterface{
	...
	public void save(Evento event, int tipo, int modalidad) {
		...
		sql="INSERT INTO evento(nombre, tipo, capacidadMinima, capacidadMaxima, costo, descripcion, edadMinima, publicoObjetivo, status, modalidad)VALUES(?,?,?,?,?,?,?,?,?,?)";
		jdbc.update(sql, event.getNombre(), tipo, event.getCapacidadMinima(), event.getCapacidadMaxima(), event.getCosto(), event.getDescripcion(), event.getEdadMinima(), event.getPublicoObjetivo(), event.getStatus(), modalidad);
	}

	@Override
	public void update(Evento event, int tipo, int modalidad) {
		// TODO Auto-generated method stub
		sql="UPDATE evento set nombre=?, tipo = ?, capacidadMinima=?, capacidadMaxima=?, costo=?, descripcion=?, edadMinima=?, publicoObjetivo=?, status=?, modalidad=? WHERE idEvento=?";
		jdbc.update(sql, event.getNombre(), tipo, event.getCapacidadMinima(), event.getCapacidadMaxima(), event.getCosto(), event.getDescripcion(), event.getEdadMinima(), event.getPublicoObjetivo(), event.getStatus(), modalidad, event.getIdEvento());
	}
